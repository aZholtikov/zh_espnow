...
{
    ZH_LOGI("Number of attempts set started.");
    ZH_ERROR_CHECK(_is_initialized == true, ESP_ERR_INVALID_STATE, NULL, "Number of attempts set failed. ESP-NOW is not initialized.");
    ZH_ERROR_CHECK(attempts > 0, ESP_ERR_INVALID_ARG, NULL, "Number of attempts set failed. Invalid number.");
    _init_config.attempts = attempts;
    ZH_LOGI("Number of attempts set successfully.");
    return ESP_OK;
}

uint8_t zh_espnow_get_channel(void)
{
    ZH_LOGI("ESP-NOW channel receiption started.");
    ZH_ERROR_CHECK(_is_initialized == true, 0, NULL, "ESP-NOW channel receiption failed. ESP-NOW is not initialized.");
    uint8_t prim_channel = 0;
    wifi_second_chan_t sec_channel = 0;
    ZH_ERROR_CHECK(esp_wifi_get_channel(&prim_channel, &sec_channel) == ESP_OK, 0, NULL, "ESP-NOW channel receiption failed.");
    _init_config.wifi_channel = prim_channel;
    ZH_LOGI("ESP-NOW channel receiption successfully.");
    return prim_channel;
}

esp_err_t zh_espnow_set_channel(uint8_t channel)
{
    ZH_LOGI("ESP-NOW set channel started.");
    ZH_ERROR_CHECK(_is_initialized == true, ESP_ERR_INVALID_STATE, NULL, "ESP-NOW channel set failed. ESP-NOW is not initialized.");
    ZH_ERROR_CHECK(channel > 0 && channel < 15, ESP_ERR_INVALID_ARG, NULL, "ESP-NOW channel set failed. Invalid channel.");
    ZH_ERROR_CHECK(esp_wifi_set_channel(channel, WIFI_SECOND_CHAN_NONE) == ESP_OK, ESP_FAIL, NULL, "ESP-NOW channel set failed.");
    _init_config.wifi_channel = channel;
    ZH_LOGI("ESP-NOW set channel completed successfully.");
    return ESP_OK;
}
...
{
    ZH_LOGI("Battery mode set started.");
    ZH_ERROR_CHECK(_is_initialized == true, ESP_ERR_INVALID_STATE, NULL, "Battery mode set failed. ESP-NOW is not initialized.");
    ZH_ERROR_CHECK(esp_now_unregister_send_cb() == ESP_OK, ESP_FAIL, NULL, "Battery mode set failed. Failed to unregister send callback.");
    if (_init_config.battery_mode == false)
    {
        ZH_ERROR_CHECK(esp_now_unregister_recv_cb() == ESP_OK, ESP_FAIL, NULL, "Battery mode set failed. Failed to unregister receive callback.");
    }
    ZH_ERROR_CHECK(_zh_espnow_callbacks_register(battery_mode) == ESP_OK, ESP_FAIL, NULL, "Battery mode set failed. Failed to register callbacks.");
    _init_config.battery_mode = battery_mode;
    ZH_LOGI("Battery mode set successfully.");
    return ESP_OK;
}
...
{
    ZH_LOGI("Queue get space started.");
    if (_queue_handle == NULL)
    {
        return 0;
    }
    ZH_LOGI("Queue get space completed successfully.");
    return uxQueueSpacesAvailable(_queue_handle);
}

void zh_espnow_clear_queue(void)
{
    ZH_LOGI("Queue clear started.");
    if (_queue_handle != NULL)
    {
        _queue_t queue = {0};
    ...
    ZH_LOGI("Queue clear completed successfully.");
}